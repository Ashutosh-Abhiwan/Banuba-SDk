...
namespace BNB
{
    ...
    public class CameraDevice : MonoBehaviour
    {
        ...
        private void UpdateEmptyImage()
        {
            ...
            _data = new Color32[texture.width * texture.height];
            _dataPinner = new Color32Pinner(_data);
            _data = texture.GetPixels32();
            if (CameraTexture) {
                Destroy(CameraTexture);
            }

            _cameraTexture = new Texture2D(texture.width, texture.height, TextureFormat.RGBA32, false);
            CameraTexture.SetPixels32(_data);
            CameraTexture.Apply();
            if (_data == null) {
            ...
            _cameraImage.format.width = (uint) texture.width;
            _cameraImage.format.height = (uint) texture.height;
            _dataPinner = new Color32Pinner(texture.GetPixels32()); // texture passed to FRX HERE
        ...
        {
            bool dataChanged = cameraTextureData.angle != _webCamTexture.videoRotationAngle
            ...
            if (_texSize != _data.Length || dataChanged) {
                UpdateTexture();
            }
            ...
            if (isLocal) {
                ...
                _webCamTexture.GetPixels32(_data); // NOTE: pixels are vertically flipped according to documentation
            }
            CameraTexture.SetPixels32(_data);
            CameraTexture.Apply();
            if (_data == null) {
